...
class BaseTrainer:
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            # RUN TRAIN (AND VAL)
            results = self._train_epoch(epoch)  # LOSS AND METRIC
            if self.do_validation and epoch % self.config['trainer']['val_per_epochs'] == 0:
                results = self._valid_epoch(epoch)

                # LOGGING INFO
                self.logger.info('\n         ## Info for epoch {} ## '.format(epoch))
                for k, v in results.items():
                    self.logger.info('         {}: {}'.format(str(k)+":15s",v))
            ...
            if self.mnt_mode != 'off' and epoch % self.config['trainer']['val_per_epochs'] == 0:
                try:
                    ...
                    self.logger.warning(
                        'The metrics being tracked ({}) has not been calculated. Training stops.'.format(self.mnt_metric))
                    break
                ...
                if self.not_improved_count > self.early_stoping:  # EARLY STOP IF DOESN'T IMPROVE LONG ENOUGH
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        # print("svae_path:{}".format(filename))
