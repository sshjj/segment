...
class _PSPModule(nn.Module):
    def __init__(self, in_channels, bin_sizes, norm_layer):  # bin_sizes 个不同的全局
    ...
    def _make_stages(self, in_channels, out_channels, bin_sz, norm_layer):
        ...
        # 平均池化输出 bin_sz 方块，N，channels 不变
    ...
    def forward(self, features):
        ...
        pyramids.extend([F.interpolate(stage(features), size=(h, w), mode='bilinear',
                                       align_corners=True) for stage in self.stages])  # 上采样到一致
        output = self.bottleneck(torch.cat(pyramids, dim=1))
        return output


class PSPNet(BaseModel):
    def __init__(self, num_classes=8, in_channels=3, backbone='resnet152',
                 pretrained=False, use_aux=True, freeze_bn=False, freeze_backbone=False):
        super(PSPNet, self).__init__()
        # TODO: Use synch batchnorm
        norm_layer = nn.BatchNorm2d
        model = getattr(resnet, backbone)(pretrained=False, norm_layer=norm_layer, )
    ...
    def forward(self, x):
        ...
        output = self.master_branch(x)  # 通过 resnet，然后四个 PSP module 全局采样后卷积
        output = F.interpolate(output, size=input_size, mode='bilinear', align_corners=True)
        output = output[:, :, :input_size[0], :input_size[1]]

        if self.training and self.use_aux:  # 从 layer3 接出一个辅助分支
...
# PSP with DenseNet as the backbone

class PSPDenseNet(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='densenet201', pretrained=False, use_aux=True,
                 freeze_bn=False, **_):
